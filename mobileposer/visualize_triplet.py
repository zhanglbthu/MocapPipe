...
def prepare_vertices_triplet(gt_vertices, cal_vertices, base_vertices, subject_spacing):
    gt_vertices = gt_vertices.copy()
    cal_vertices = cal_vertices.copy()
    base_vertices = base_vertices.copy()

    all_vertices = np.concatenate(
        [
            gt_vertices.reshape(-1, 3),
            cal_vertices.reshape(-1, 3),
    ...
    gt_vertices[:, :, 0] -= subject_spacing
    base_vertices[:, :, 0] += subject_spacing
    return gt_vertices, cal_vertices, base_vertices


class TripletRenderer:
    def __init__(self, faces, width, height, fps, face_stride=1, subject_spacing=1.2):
    ...
    def _make_meshes(self, gt_vertices, cal_vertices, base_vertices):
        gt_vertices, cal_vertices, base_vertices = prepare_vertices_triplet(
            gt_vertices,
            cal_vertices,
            base_vertices,
        ...
        specs = [
            ...
            ("Calibrated", cal_vertices, tuple(float(x) for x in colors[2]) + (1.0,)),
    ...
    def render_video(self, gt_vertices, cal_vertices, base_vertices, output_path):
        nodes = self._make_meshes(gt_vertices, cal_vertices, base_vertices)
        ...
        try:
            ...
            viewer.scene.camera.position = np.array([0.0, 0.2, 6.6], dtype=np.float32)
...
def main():
    ...
    parser.add_argument("--calibrated-dir", type=str, required=True)
    ...
    n_frames = min(len(gt_vertices), len(cal_vertices), len(base_vertices))
    frame_ids = list(range(0, n_frames, args.stride))
    if args.max_frames is not None:
        frame_ids = frame_ids[:args.max_frames]

    gt_vertices = gt_vertices[frame_ids]
    cal_vertices = cal_vertices[frame_ids]
    ...
    renderer.render_video(gt_vertices, cal_vertices, base_vertices, output_path)
