...
from gem.utils.geo_transform import transform_mat
...
def _axis_rotate_to_matrix(angle, axis="y"):
    """Create rotation matrix from angle and axis."""
    if axis == "y":
        aa = torch.tensor([0.0, float(angle), 0.0])
    elif axis == "x":
        aa = torch.tensor([float(angle), 0.0, 0.0])
    elif axis == "z":
        aa = torch.tensor([0.0, 0.0, float(angle)])
    else:
...
def noisy_interpolation(x, length, step_noise_perc=0.2):
    ...
    # Use linspace(0, 1) +- noise as reference
...
def noisy_impluse_interpolation(data1, data2, step_noise_perc=0.2):
    """Non-linear interpolation of impulse with noise"""
...
def create_camera(w_root, cfg):
    ...
    yaw_rm = _axis_rotate_to_matrix(yaw, axis="y")
    pitch_rm = _axis_rotate_to_matrix(pitch, axis="x")
    roll_rm = _axis_rotate_to_matrix(roll, axis="z")
...
def create_rotation_move(R, length, r_xyz_w_std=None):
    ...
    Return:
        R_move: (L, 3, 3)
    """
    if r_xyz_w_std is None:
        r_xyz_w_std = [np.pi / 8, np.pi / 4, np.pi / 8]
...
def create_translation_move(R_w2c, t_w2c, length, t_xyz_w_std=None):
    """Create translational move for the camera
    Args:
        R_w2c: (3, 3),
        t_w2c: (3,),
    """
    if t_xyz_w_std is None:
        t_xyz_w_std = [1.0, 0.25, 1.0]
...
class CameraAugmentorV11:
    ...
    def add_stepnoise(self, R, T):
        ...
        if impulse_type == "t":
            # impulse translation only
            T_new = add_impulse_t(T_new)
        elif impulse_type == "r":
            # impulse rotation only
            R_new = add_impulse_rot(R_new)
        elif impulse_type == "both":
            # impulse rotation and translation
            R_new = add_impulse_rot(R_new)
            T_new = add_impulse_t(T_new)
        else:
